...
### AI Shopping Assistant (Phase 10)
The assistant bubble appears bottom-left on every page. It is currently in **STUB MODE** (no Anthropic API key set). To go live:
1. Add `ANTHROPIC_API_KEY=sk-ant-...` to the backend service in Railway → Variables.
2. Redeploy the backend (no code changes needed). `/health` will return `"assistant":"live"`.
3. Run the deferred adversarial tests (Step 6f from the Phase 10 spec) before announcing the live assistant to customers.

**What it costs:** claude-haiku-4-5-20251001 is the cheapest Claude model. Token usage is logged per call (`[assistant] tokens: input=X output=Y`). Default daily cap is 1,000 calls/day (configurable via `ASSISTANT_DAILY_CAP` env var). After the cap, the endpoint returns a polite "resting" message instead of calling the API.

**Rate limits:** 15 messages per IP per hour for the assistant; 3 product-request submissions per IP per hour. Both are in-process (same known multi-instance caveat as Phase 6 — acceptable at single-container scale).
...
### Parked debt
1. **In-memory rate limiting (per-container):** The login, vendor-image, vendor-application, and assistant rate limiters use in-process Maps. If Railway scales to multiple containers, each has its own counter. At current scale (single container) this is acceptable. First step to close: add a Redis service to Railway and switch to a Redis store.
