...
| 2026-07-29 | Phase 2 | TBD | **Phase 2 COMPLETE.** Production domain everythingbida.com live on Vercel (external DNS at Namecheap, A @ 216.198.79.1 + CNAME www → 215f4a2270340af2.vercel-dns-017.com., records unchanged). Delta migration: zero new rows in Netlify Blobs since Phase 2B snapshot; final snapshot 20260729T114750Z saved to backup/. Netlify code retired: netlify/functions/api.mjs, netlify/ dir, netlify.toml deleted; @netlify/blobs + netlify-cli removed from package.json. www→apex 308 redirect via vercel.json. TLS issued (Let's Encrypt). NOTE: bank_settings must be populated by operator via admin panel. Phase 3 (delivery locations) is next. |
